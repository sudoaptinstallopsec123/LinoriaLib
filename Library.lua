...
writefile("Scrollbar.png", game:HttpGet("https://github.com/sametexe001/images/blob/main/scrollbar.png?raw=true"))
...
function Library:GetTextBounds(Text, FontFace, Size, Resolution)
	...
	Params.Text = Text
	Params.Font = FontFace or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular)
	Params.Size = Size
...
function Library:CreateWindow(...)
	...
	function Window:AddTab(Name)
		...
		function Tab:AddScrollbox(Info)
			...
			local Container = Library:Create("ScrollingFrame", {
				...
				MidImage = getcustomasset("Scrollbar.png"),
                TopImage = getcustomasset("Scrollbar.png"),
                BottomImage = getcustomasset("Scrollbar.png"),
				BorderSizePixel = 0,
				ZIndex = 5,
				Parent = BoxInner,
			})
			...
			local function UpdateCanvas()
				...
				Container.ScrollBarThickness = ContentHeight > Container.AbsoluteSize.Y and 1 or 0
			end

			Layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
				Container.CanvasSize = UDim2.new(0, 0, 0, Layout.AbsoluteContentSize.Y)
				Container.ScrollBarThickness = Layout.AbsoluteContentSize.Y > Container.AbsoluteSize.Y and 1 or 0
			end)
	...
	function Library:Toggle()
		...
		MenuBlur.Enabled = getgenv().MenuBlurEnabled
		...
		if Toggled then
			...
			BlurTween = TweenService:Create(MenuBlur, TweenInfo.new(FadeTime, Enum.EasingStyle.Linear), { Size = 20 })
			BlurTween:Play()
